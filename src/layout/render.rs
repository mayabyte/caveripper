...
pub fn render_layout(layout: &Layout) {
    let mut image_buffer = ImageBuffer::from_pixel(512, 512, Rgb::from([0,0,0]));
    for map_unit in layout.map_units.iter() {
        ...
        for (radar_x, radar_z, pixel) in radar_image.enumerate_pixels() {
            image_buffer.put_pixel((img_x + 256) as u32 + radar_x, (img_z + 256) as u32 + radar_z, pixel.clone());
        }
    }
