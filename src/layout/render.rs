...
pub fn render_layout(layout: &Layout) {
    ...
    for map_unit in layout.map_units.iter() {
        ...
        let radar_image_bytes = get_file_bytes(&format!("assets/arc/{}/arc.d/texture.bti.png", map_unit.unit.unit_folder_name)).unwrap();
    ...
    for door in layout.map_units.iter().flat_map(|unit| unit.doors.iter()) {
        match door.borrow().seam_spawnpoint {
            ...
            Some(SpawnObject::Gate(_)) => {
                ...
                image_buffer.put_pixel(x, z, Pixel::from_channels(50, 50, 255, 255));
            },
