...
impl<'a> Renderer<'a> {
    pub fn new(mgr: &'a AssetManager) -> Self {
        ...
        Self {
            mgr,
            fonts: vec![read_font("resources/BalooChettan2-SemiBold.ttf"), read_font("resources/BalooChettan2-ExtraBold.ttf")]
        }
    }
    ...
    pub fn render_caveinfo(&self, caveinfo: &CaveInfo, options: CaveinfoRenderOptions) -> Result<RgbaImage, CaveripperError> {
        ...
        // Sublevel name
        let sublevel_title = self.render_text(&caveinfo.long_name(), 64.0, [0,0,0, 255].into(), None);
        ...
        metadata_icons.push(
            resize(
                SpawnObject::Ship.get_texture(&caveinfo.cave_cfg.game, self.mgr)
                    .change_context(CaveripperError::RenderingError)?
                    .as_ref(),
        ...
        for gateinfo in caveinfo.gate_info.iter() {
            let gate_icon = resize(
                gateinfo.get_texture(&caveinfo.cave_cfg.game, self.mgr)
                    .change_context(CaveripperError::RenderingError)?
                    .as_ref(),
                CAVEINFO_ICON_SIZE, CAVEINFO_ICON_SIZE, FilterType::Lanczos3);
            let num_txt = self.render_small_text(&format!("x{num_gates}"), 19.0, [20, 20, 20, 255].into());
            let hp_txt = self.render_small_text(&format!("{}HP", gateinfo.health as u32), 13.0, [20, 20, 20, 255].into());
        ...
        let poko_icon = resize(
            self.mgr.get_img("resources/enemytex_special/Poko_icon.png").change_context(CaveripperError::RenderingError)?,
            16, 19, FilterType::Lanczos3);

        // Teki section
        let mut base_y =  64 + CAVEINFO_MARGIN * 2;
        let teki_header = self.render_text(&format!("Teki (max {})", caveinfo.max_main_objects), 48.0, [225,0,0, 255].into(), None);
        ...
        for group in [8, 1, 0, 6, 5] {
            for tekiinfo in caveinfo.teki_group(group) {
                let texture = resize(
                    tekiinfo.get_texture(&caveinfo.cave_cfg.game, self.mgr)
                        .change_context(CaveripperError::RenderingError)?
                        .as_ref(),
                    CAVEINFO_ICON_SIZE, CAVEINFO_ICON_SIZE, FilterType::Lanczos3
                );
                    ...
                    match modifier {
                        TextureModifier::Falling => {
                            let falling_icon_texture = resize(
                                self.mgr.get_img("resources/enemytex_special/falling_icon.png").change_context(CaveripperError::RenderingError)?,
                        ...
                        TextureModifier::Carrying(carrying) => {
                            ...
                            let carried_treasure_icon = resize(
                                self.mgr.get_img(
                                    &PathBuf::from_iter(["assets", &caveinfo.cave_cfg.game, "treasures", &format!("{carrying}.png")])
                                ).change_context(CaveripperError::RenderingError)?,
                            ...
                            if options.draw_treasure_info {
                                let value_text = self.render_text(&format!("{}", treasure.value), 20.0, [20,20,20, 255].into(), None);
                                let carriers_text = self.render_text(&format!("{}/{}", treasure.min_carry, treasure.max_carry), 20.0, [20, 20, 20, 255].into(), None);
                ...
                let teki_subtext_texture = self.render_text(&teki_subtext, 24.0, subtext_color, None);
                overlay(
                    &mut canvas_header,
        ...
        for treasureinfo in caveinfo.item_info.iter() {
            ...
            let treasure_texture = resize(
                treasureinfo.get_texture(&caveinfo.cave_cfg.game, self.mgr)
                    .change_context(CaveripperError::RenderingError)?
                    .as_ref(),
                CAVEINFO_ICON_SIZE, CAVEINFO_ICON_SIZE, FilterType::Lanczos3
            );
            ...
            if caveinfo.is_challenge_mode() {
                ...
                let treasure_subtext_texture = self.render_text(&treasure_subtext, 24.0, subtext_color, None);
                overlay(
                    &mut canvas_header,
        ...
        for (i, capinfo) in caveinfo.cap_info.iter().enumerate() {
            let texture = resize(
                capinfo.get_texture(&caveinfo.cave_cfg.game, self.mgr)
                    .change_context(CaveripperError::RenderingError)?
                    .as_ref(),
                CAVEINFO_ICON_SIZE, CAVEINFO_ICON_SIZE, FilterType::Lanczos3
            );
            ...
            let capteki_subtext_texture = self.render_text(&capteki_subtext, 24.0, capteki_color, None);
        ...
        for (i, unit) in caps.iter().enumerate() {
            ...
            for spawnpoint in unit.spawnpoints.iter() {
                ...
                let sp_img = match spawnpoint.group {
                    6 => colorize(resize(self.mgr.get_img("resources/enemytex_special/leaf_icon.png")
                        .change_context(CaveripperError::RenderingError)?, 10, 10, FilterType::Lanczos3), group_color(6).into()),
        ...
        for unit in rooms {
            ...
            let unit_name_text = self.render_text(&unit.unit_folder_name, 14.0, [220,220,220,255].into(), Some(unit_texture.width()));
            ...
            if options.draw_waypoints {
                for waypoint in unit.waypoints.iter() {
                    ...
                    overlay(unit_texture.to_mut(), &wp_img, wp_pos[0] as i64 - (wp_img.width() / 2) as i64, wp_pos[2] as i64 - (wp_img.height() / 2) as i64);
                    ...
                    for link in waypoint.links.iter() {
                        ...
                        draw_arrow_line(unit_texture.to_mut(), wp_pos.into(), dest_wp_pos.into(), CARRY_PATH_COLOR.into());
            ...
            for spawnpoint in unit.spawnpoints.iter().sorted_by_key(|sp| sp.group) {
                ...
                let sp_img = match spawnpoint.group {
                    ...
                    2 => colorize(resize(self.mgr.get_img("resources/enemytex_special/duck.png").change_context(CaveripperError::RenderingError)?, 14, 14, FilterType::Lanczos3), group_color(2).into()), // treasure
                    4 => resize(self.mgr.get_img("resources/enemytex_special/cave_white.png").change_context(CaveripperError::RenderingError)?, 18, 18, FilterType::Lanczos3),
                    6 => colorize(resize(self.mgr.get_img("resources/enemytex_special/leaf_icon.png").change_context(CaveripperError::RenderingError)?, 10, 10, FilterType::Lanczos3), group_color(6).into()),
                    7 => resize(self.mgr.get_img("resources/enemytex_special/ship.png").change_context(CaveripperError::RenderingError)?, 16, 16, FilterType::Lanczos3),
                    8 => colorize(resize(self.mgr.get_img("resources/enemytex_special/star.png").change_context(CaveripperError::RenderingError)?, 16, 16, FilterType::Lanczos3), group_color(8).into()),
                    _ => circle(5, [255,0,0,255].into()),
                };

                overlay(unit_texture.to_mut(), &sp_img, sp_x - (sp_img.width() / 2) as i64 , sp_z - (sp_img.height() / 2) as i64);
            }
    ...
    fn draw_object_at<Tex: Textured>(&self, image_buffer: &mut RgbaImage, obj: &Tex, pos: Point<2,f32>, game: &str, options: &LayoutRenderOptions) -> Result<(), CaveripperError> {
        ...
        for modifier in obj.get_texture_modifiers().iter() {
            match modifier {
                TextureModifier::Falling => {
                    let falling_icon_texture = resize(
                        self.mgr.get_img("resources/enemytex_special/falling_icon.png")?,
                ...
                TextureModifier::Carrying(carrying) => {
                    let carried_treasure_icon = resize(
                        self.mgr.get_img(&PathBuf::from_iter(["assets", game, "treasures", &format!("{carrying}.png")]))?,
...
impl Textured for TekiInfo {
    fn get_texture<'a>(&self, game: &str, mgr: &'a AssetManager) -> Result<Cow<'a, RgbaImage>, CaveripperError> {
        match get_special_texture_name(&self.internal_name) {
            ...
            None => {
                let filename = PathBuf::from_iter(["assets", game, "teki", &format!("{}.png", self.internal_name.to_ascii_lowercase())]);
                Ok(Cow::Borrowed(mgr.get_img(filename)?))
            }
        }
    }
...
impl Textured for ItemInfo {
    fn get_texture<'a>(&self, game: &str, mgr: &'a AssetManager) -> Result<Cow<'a, RgbaImage>, CaveripperError> {
        let filename = PathBuf::from_iter(["assets", game, "treasures", &format!("{}.png", self.internal_name.to_ascii_lowercase())]);
        Ok(Cow::Borrowed(mgr.get_img(filename)?))
    }
...
impl Textured for GateInfo {
    fn get_texture<'a>(&self, _game: &str, mgr: &'a AssetManager) -> Result<Cow<'a, RgbaImage>, CaveripperError> {
    ...
    fn get_texture<'a>(&self, game: &str, mgr: &'a AssetManager) -> Result<Cow<'a, RgbaImage>, CaveripperError> {
        match self {
            ...
            SpawnObject::Gate(gateinfo, rotation) => {
                let texture = gateinfo.get_texture(game, mgr)
                    .change_context(CaveripperError::RenderingError)?;
                if rotation % 2 == 1 {
            ...
            SpawnObject::Hole(plugged) => {
                ...
                if !plugged {
                    ...
                    Ok(Cow::Borrowed(mgr.get_or_store_img("PLUGGED_HOLE".to_string(), || {
            ...
            SpawnObject::Geyser(plugged) => {
                ...
                if !plugged {
                    ...
                    Ok(Cow::Borrowed(mgr.get_or_store_img("PLUGGED_GEYSER".to_string(), || {
                        let mut hole_icon = mgr.get_img(filename)?.clone();
                        let plug_filename = "resources/enemytex_special/36px-Clog_icon.png";
                        let plug_icon = resize(
                            mgr.get_img(plug_filename)?,
...
impl Textured for CaveUnit {
    fn get_texture<'a>(&self, game: &str, mgr: &'a AssetManager) -> Result<Cow<'a, RgbaImage>, CaveripperError> {
        ...
        Ok(Cow::Borrowed(mgr.get_or_store_img(
            format!("{}_r{}_prerendered", filename.to_string_lossy(), self.rotation),
            Box::new(|| Ok(img))
        )?))
    }
